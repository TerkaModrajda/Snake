...
function animate        const attractionStrength = 1.0; // Síla přitažení (1 pole za frame)onst head = gameState.snake[0];
    const attractionRange = 2; // Rozmezí 4x4 (2 pole na všechny strany od hlavy)
    const attractionStrength = 1.0; // Síla přitažení (1 pole za frame)re() {
...
function updateMagnetAttraction() {
    ...
    console.log('Magnet efekt aktivní, had na pozici:', head.x, head.y);
    
    // Přitáhni hlavní jídlo
    if (gameState.food) {
        console.log('Před přitažením - jídlo na pozici:', gameState.food.x, gameState.food.y);
        const newFood = attractFoodToSnake(gameState.food, head, attractionRange, attractionStrength);
        if (newFood.x !== gameState.food.x || newFood.y !== gameState.food.y) {
            console.log('Jídlo se pohnulo z', gameState.food.x, gameState.food.y, 'na', newFood.x, newFood.y);
        }
...
function attractFoodToSnake(food, head, maxRange, strength) {
    ...
    if (dx <= 1 && dy <= 1) {
        console.log('Jídlo je příliš blízko, neovlivňuji');
        return food;
    }
    
    console.log('Jídlo je v dosahu, přitahuji!');
